...
class Node:
	...
	def formula(self,x,y):
		...
		for i in range(len(x)):
			temp=(x[i]/y)
			p = p - (temp * (np.log(temp)/np.log(2)))
		return p
	
	def entropy(self,all_words, my_words_idx,query_idx):
		ml = len(my_words_idx)
	...
	def myf(self,my_words_idx,words):
		...
		for i in my_words_idx: 
			query_idx = i
			ml = len(my_words_idx)
			dict1 = (self.entropy(words,my_words_idx,query_idx))
			dict2 = []
			for j in dict1:
				dict2.append(len(dict1[j]))
			value = self.formula(dict2,ml)
			if value > maxi:
				maxi = value
				z = dict1
			if value not in dd:
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		if len( history ) == 0:
			...
			query_idx,z = self.myf(my_words_idx,all_words)
			query = all_words[ query_idx ]
